...
function parseSidebarLists(nodes) {
  ...
  const hosts = nodes.filter(n => !n.id.includes('router') && !n.id.includes('switch')); // If any
  ...
  const populate = (listEl, items) => {
    ...
    items.forEach(item => {
      ...
      li.onclick = () => {
        ...
        if(nodeRef) {
          appState.setState({ view: 'node', selectedNode: nodeRef, selectedPort: null });
        }
      };
...
appState.subscribe((state) => {
  ...
  if (state.view === 'main') {
    ...
    setTimeout(() => resizeGraph('graph-container'), 100);
  } 
    ...
    bottomPanelTitle.textContent = `Connected Ports`;
    
    // Find connected links
    const sn = state.selectedNode;
    const connectedLinks = state.links.filter(l => l.source.id === sn.id || l.target.id === sn.id);
    
    // Render links as a table
    if(connectedLinks.length === 0) {
      bottomPanelContent.innerHTML = `<p>No connections found.</p>`;
    } else {
    ...
    const targetLabel = sp.source === state.selectedNode ? sp.target.label : sp.source.label;
...
btnGetConfig.addEventListener('click', async () => {
  ...
  setTimeout(() => {
    ...
    initGraph('graph-container', data.nodes, data.edges, (node) => {
      appState.setState({ view: 'node', selectedNode: node, selectedPort: null });
    });
  }, 50);
});
